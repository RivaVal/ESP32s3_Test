 ...
 * @file unified_ledc.cpp
 * @brief Реализация контроллера моторов через LEDC ESP32-S3
 * @version 2.1.0 (Интеграция с Config::Pins)
 */
...
bool UnifiedLEDCController::begin() {
    ...
    ESP_LOGI(TAG, "=================================================");
    ESP_LOGI(TAG, "⚙️ ИНИЦИАЛИЗАЦИЯ LEDC (ESP32-S3 Native PWM)");
    ESP_LOGI(TAG, "=================================================");
    ESP_LOGI(TAG, "📍 Пины моторов (из Config::Pins):");
    ESP_LOGI(TAG, "   • Motor LEFT  -> GPIO%d (LEDC CH%d)", 
             Config::Pins::MOTOR_LEFT_PIN, Config::Pins::LEDC_CH_LEFT);
    ESP_LOGI(TAG, "   • Motor RIGHT -> GPIO%d (LEDC CH%d)", 
             Config::Pins::MOTOR_RIGHT_PIN, Config::Pins::LEDC_CH_RIGHT);
    ESP_LOGI(TAG, "⚡ Параметры ШИМ: %lu Гц | %lu бит (0-%lu)", 
             _freqHz, _dutyResolution, (1UL << _dutyResolution) - 1);

    // 1️⃣ Настройка общего таймера
...
bool UnifiedLEDCController::setMotorPower(uint8_t motorIndex, float powerPercent) {
    ...
    ledc_channel_t ch = (motorIndex == 0) ? 
                        static_cast<ledc_channel_t>(Config::Pins::LEDC_CH_LEFT) : 
                        static_cast<ledc_channel_t>(Config::Pins::LEDC_CH_RIGHT);

    esp_err_t err = ledc_set_duty(_speedMode, ch, duty);
    if (err != ESP_OK) return false;

    err = ledc_update_duty(_speedMode, ch);
    if (err != ESP_OK) return false;

    ESP_LOGV(TAG, "🔋 Motor #%u: %.1f%% → duty=%lu/1023", motorIndex, powerPercent, duty);
    return true;
}
...
esp_err_t UnifiedLEDCController::setupTimer() {
    ...
    timer_conf.speed_mode      = _speedMode;
    timer_conf.timer_num       = static_cast<ledc_timer_t>(Config::Pins::LEDC_TIMER_NUM);
...
esp_err_t UnifiedLEDCController::setupChannel(uint8_t index) {
    ...
    // 🔑 БЕРЕМ ПИНЫ И КАНАЛЫ ИЗ ГЛОБАЛЬНОГО КОНФИГА!
    ch_conf.gpio_num     = (index == 0) ? Config::Pins::MOTOR_LEFT_PIN : Config::Pins::MOTOR_RIGHT_PIN;
    ch_conf.speed_mode   = _speedMode;
    ch_conf.channel      = (index == 0) ? static_cast<ledc_channel_t>(Config::Pins::LEDC_CH_LEFT) 
                                        : static_cast<ledc_channel_t>(Config::Pins::LEDC_CH_RIGHT);
    ch_conf.timer_sel    = static_cast<ledc_timer_t>(Config::Pins::LEDC_TIMER_NUM);
    ch_conf.duty         = 0; // Старт с 0%
    ch_conf.hpoint       = 0;

    esp_err_t err = ledc_channel_config(&ch_conf);
    if (err == ESP_OK) {
        ledc_set_duty(_speedMode, ch_conf.channel, 0);
        ESP_LOGI(TAG, "✅ Канал %d настроен: GPIO%d (LEDC CH%d)", 
                 index, ch_conf.gpio_num, ch_conf.channel);
    }
    return err;
}
